...
- Запрос на nginx, тот на основе выбранного алгоритма выбирает микросервис (условно назовем его первый) для формирования ленты. Перед тем как сформировать ленту, запрос попадает на второй микросервис, который пробрасывает запрос на шарды БД, описанные в п.5, далее выбранные посты возвращаются на первый микросервис и отдается на фронтенд.
