...
Фронтенд: средний собранный фронтенд занимает 2-5 Мб, для определенности возьмем 3Мб. 
- [данные о vk](https://popsters.ru/blog/post/svezhie-dannye-o-vk)
- [фронтенд](https://habr.com/ru/company/tinkoff/blog/474632/)
- 3 Мб * N количество польз в час = K Гбит/с
Бекенд:
...
Для обеспечения отказоустойчивости как уже было сказано в п.5 на каждый master будем хранить slave. Таким образом чтение будет идти со slave, а запись в master. Так как между данными в master и в slave может быть задержка, в последовательных операциях чтения данные будут идти с master. Если один из master или slave ломается, переключаем работу со slave на master и наооборот, настраивая новую репликацию. Таким образом нет единой точки отказа. Кроме того, для предсказания деградации приложения на каком-либо сервере или приложения в целом будем собирать метрики, отдавать метрики по /metrics с помощью prometheus и отрисовывать графики пользуясь graphana.
