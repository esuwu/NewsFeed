...
Достаточно будет 4 серверов:

| CPU(cores) | RAM(GB) | SSD(GB) |
|------------|---------|---------|
|   16       | 32      | 512    |

Возьмем по 2 с запасом на каждый = 12 серверов.

### Бекенд:
Из 1 291 666 RPS, приходится ~41 000 RPS на одно лишь формирование ленты на бекенде. В у среднестатистического пользователя такая операция занимает ~250мс, за секунду 4 ядерный сервер в целом может держать 1 000 RPS. Учитывая наши 41 000 RPS нам понадобится 41 сервер следующих характеристик:
| CPU(cores) | RAM(GB) | SSD(GB) |
|------------|---------|---------|
|   16       | 16      | 512    |
...
Нам достаточно 3 серверов, для надежности возьмем с запасом 10.
| CPU(cores) | RAM(GB) | SSD(GB) |
|------------|---------|---------|
|   16       | 32      | 512    |
...
Пользователи сервиса - русскоязычные поэтому возьмем наиболее близкие возможные расположения. Для равномерного распределения по ДЦ поделим общее количество серверов на 4 и на каждом ДЦ поставим по ~ 3 сервера с фронтендом, 15 серверов с бекендом, 353 сервера с БД и по 3 сервера с балансировщиком.  
- Европа (Стокгольм)
- Китай (Пекин)
- Франция (Париж)
- Милан
