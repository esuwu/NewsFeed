...
Выходит что наших 215 серверов (не считая slave) недостаточно. Если один сервер имеет память 4096 Тб, то нам понадобится 762 сервера примерно по 2000 RPS на 1, не считая slaves. Вместе со slaves получится 1524 сервера.


Nginx:
Согласно этому [Performance](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/)
Нам достаточно такого сервера, с запасом возьмем 2.
| CPU(cores) | RAM(GB) | SSD(GB) |
|------------|---------|---------|
|   16       | 32      | 256    |
...
Пользователи сервиса - русскоязычные поэтому возьмем наиболее близкие возможные расположения
- Европа (Стокгольм)
- Китай (Пекин)
- Франция (Париж)
- Милан

В случае отказа одного из дата центра, весь наш сервер не упадет, тк выбрали сервера из разных зон.

## 9. **Схема балансировки нагрузки (входящего трафика и внутрипроектного, терминация SSL)**
Для балансировки нагрузки будем использовать DNS-балансировку + nginx со встроенными настройками SSL терминации и схему L7. Балансировка нагрузки по классическому Round Robin алгоритму - нет зависимости от времени пакета, более менее равномерное распределение ресурсов. 
